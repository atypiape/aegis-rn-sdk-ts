...
```bash
$ npm install --save aegis-rn-sdk-ts
```
...
```typescript
import Aegis, { AegisConfig } from 'aegis-rn-sdk-ts';
...
官方 [aegis-rn-sdk](https://www.npmjs.com/package/aegis-rn-sdk) 在运行时，内部会抛出异常 `ReferenceError: Property 'location' doesn't exist`。本库屏蔽了 `LogBox` 弹窗，并中断该异常消息的上报。

### 3. 过滤 React Native 网络请求

`React Native` 有时会自动对 `clients3.google.com` 发送网络请求，但由于“被墙”导致请求失败，从而触发 `Aegis` 上报异常消息。本库中断该异常消息的上报。

### 4. 替换 AsyncStorage

`AsyncStorage` 已经过时，将在未来的 `React Native` 新版本中废弃。因此，本库将 `AsyncStorage` 替换为 `@react-native-async-storage/async-storage`。
...
```javascript
import Aegis from 'aegis-rn-sdk-ts';
